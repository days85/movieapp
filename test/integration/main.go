...
func main() {
	...
	regitry := memory.NewRegistry()

	log.Println("Setting up service handlers and clients")

	metadataSrv := startMetadataService(ctx, regitry)
	defer metadataSrv.GracefulStop()
	ratingSrv := startRatingService(ctx, regitry)
	defer ratingSrv.GracefulStop()
	movieSrv := startMovieService(ctx, regitry)
	...
	getInitialAggregatedRatingResp, err := ratingClient.GetAggregatedRating(ctx, &gen.GetAggregatedRatingRequest{
		RecordId:   m.Id,
		RecordType: recordTypeMovie,
	})
	if err != nil {
	...
	if got, want := getInitialAggregatedRatingResp.RatingValue, float64(5); got != want {
	...
	getNewAggregatedRatingResp, err := ratingClient.GetAggregatedRating(ctx, &gen.GetAggregatedRatingRequest{
		RecordId:   m.Id,
		RecordType: recordTypeMovie,
	})
	if err != nil {
	...
	if got, want := getNewAggregatedRatingResp.RatingValue, wantRating; got != want {
		log.Fatalf("rating mismatch: got %v, want %v", got, want)
	}

	log.Println("Getting updated movie details via movie service")

	getNewMovieDetailsResp, err := movieClient.GetMovieDetails(ctx, &gen.GetMovieDetailsRequest{MovieId: m.Id})
	if err != nil {
		log.Fatalf("get movie details: %v", err)
	}
	wantMovieDetails.Rating = wantRating
	if diff := cmp.Diff(getNewMovieDetailsResp.MovieDetails, wantMovieDetails, cmpopts.IgnoreUnexported(gen.MovieDetails{}, gen.Metadata{})); diff != "" {
